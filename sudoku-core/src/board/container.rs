...
use crate::index::{CellIndex, DigitIndex, HouseIndex, HouseRegion, SudokuRegion, SudokuSubCellIndex, SudokuIndex};
...
impl SudokuBoard {
    ...
    pub fn enumerate_house<Idx: HouseRegion>(&self, idx: &Idx) -> impl Iterator<Item=(SudokuIndex, &SudokuCell)> {
        idx.cell_indices().enumerate().map(|(i, cell_idx)| (SudokuIndex::new(i).unwrap(), &self[cell_idx]))
    }
    ...
    pub fn primaries_in<Idx: SudokuRegion>(&self, region: &Idx) -> impl Iterator<Item = (CellIndex, DigitIndex)> {
    ...
    pub fn bivalues_in<Region: SudokuRegion>(&self, region: &Region) -> impl Iterator<Item = (CellIndex, DigitFlags)> {
    ...
    pub fn indexed_iter(&self) -> impl Iterator<Item = (CellIndex, &SudokuCell)> {
        self.0.iter().enumerate().map(|(i, cell)| (CellIndex::from_flat(i).unwrap(), cell))
    }
    pub fn indexed_iter_mut(&mut self) -> impl Iterator<Item = (CellIndex, &mut SudokuCell)> {
        self.0.iter_mut().enumerate().map(|(i, cell)| (CellIndex::from_flat(i).unwrap(), cell))
    }
    ...
    pub fn is_solved(&self) -> bool {
        HouseIndex::iter().all(|h|{
            let set_digits = self.region(&h).filter_map(|cell| cell.digit_value())
